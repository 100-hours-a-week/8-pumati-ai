...
각 문장/기능이 끝날 때마다 반드시 줄바꿈(\n)과 마크다운으로 가독성 좋게 나타내줘.  `•` 또는 `-` 기호로 시작하는 목록 형식으로 나열하거나, 문단 구분을 줄바꿈으로 해줘.
...
- 각 기능 또는 문장은 `-` 기호로 시작하고, 각 문장/기능이 끝날 때마다 반드시 줄바꿈을 해줘.
- 문단을 나눌 때는 두 번의 줄바꿈을 사용해줘.
